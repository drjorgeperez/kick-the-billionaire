...
export function getMeshByName(model, name) {
  let mesh;
  model.traverse((part) => {
    if (part.isMesh && part.name === name) {
      mesh = part;
    }
  });
  return mesh;
...
export function getBoneByName(model, name) {
  let bone;
  model.traverse((part) => {
    if (part.isBone && part.name === name) {
      bone = part;
    }
  });
  return bone;
...
export function getCubeDimensionsFromMesh(mesh) {
  if (!mesh) return;
...
export function createPlaceholderMesh(meshName = "placeholder") {
  const cubeScale = {
    x: 0.1,
    y: 0.1,
    z: 0.1,
  ...
  const cubeMaterial = new THREE.MeshPhongMaterial({
    color: COLORS.BLENDER_GRAY,
  });
  const placeholderMesh = new THREE.Mesh(cubeGeometry, cubeMaterial);
  placeholderMesh.name = meshName;
  return {
    meshName: placeholderMesh.name,
    mesh: placeholderMesh,
    dimensions: getCubeDimensionsFromMesh(placeholderMesh),
  };
}

export function createMeshWithEdges(
  geometry,
